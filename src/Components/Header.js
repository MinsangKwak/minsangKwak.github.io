...
import { Bars3Icon, HomeIcon, EnvelopeIcon, ArrowLeftIcon, UserIcon, XMarkIcon } from "@heroicons/react/24/outline";
...
const Header = ({ openSidebar, isSidebarOpen }) => {
    ...
    return (
        <>
            ...
            {/* Login Modal */}
            {isLoginModalVisible && (
                <div className={`fixed inset-0 flex items-center justify-center z-50 bg-gradient-rainbow ${isFadingOut ? "animate-fade-out" : "animate-fade-in-background"} backdrop-blur-sm`}>
                    <div className={`relative bg-white/80 backdrop-blur-sm rounded-lg p-6 shadow-lg w-80 border border-white/20 ${isFadingOut ? "" : "animate-slide-up"}`}>
                        {/* Close Button */}
                        <button
            ...
            {isToastVisible && (
                <div className="fixed bottom-4 w-full px-6 z-50">
                    <div className="bg-gradient-rainbow max-w-3xl mx-auto shadow-lg rounded-lg p-4 animate-fade-in-out backdrop-blur-md border flex justify-between items-center">
                        <p className="text-white text-sm sm:text-base">요청사항 건의하기</p>
                        <a
